...
/***/ function(module, exports, __webpack_require__) {
	...
	var _computerMove2 = __webpack_require__(162);
	...
	var BoardContainer = function (_Component) {
		...
		_createClass(BoardContainer, [{
			key: 'onCellClick',
			value: function onCellClick(rowM, colM) {
				// if the move is invalid, or game is over, click does nothing
				if (!(0, _utils.isValidMove)({ row: rowM, col: colM }, this.state.boardMatrix) || this.state.winner !== null) return;
				this.playerMove(rowM, colM);
			}
		}, {
			key: 'playerMove',
			value: function playerMove(rowC, colC) {
				var winner = this.state.winner,
				    board = this.state.boardMatrix.map(function (row) {
					return row.slice();
				});
				// mark player move on matrix
				board[rowC][colC] = 'X';

				var hasPlayerWon = (0, _utils.isGameWon)(board, 'X');
				// check if that was winning play or created a tie
				if ((0, _utils.isCatsGame)(board) || hasPlayerWon) {
					winner = hasPlayerWon ? 'Player' : 'Cats Game'; // set winner
				} else {
					// get computer move coordinates
					var _computerMove = (0, _computerMove2.computerMove)(board),
					    row = _computerMove.row,
					    col = _computerMove.col;
					// mark computer move
					...
					var hasComputerWon = (0, _utils.isGameWon)(board, 'O');
					if (hasComputerWon) {
						winner = 'Computer'; // set winner
					}
				}
				this.setState({ boardMatrix: board, winner: winner });
			}
		}, {
			key: 'resetGame',
			...
			value: function render() {
				...
				return _react2.default.createElement(
					...
					null,
					this.displayTitle(),
					this.state.boardMatrix.map(function (row, rowIndex) {
						return _react2.default.createElement(
							'div',
							...
							row.map(function (cell, colIndex) {
								return _react2.default.createElement(
									'div',
									{ key: colIndex, className: 'cell', onClick: function onClick() {
											return _this3.onCellClick(rowIndex, colIndex);
										} },
									_react2.default.createElement(
										'span',
										{ className: 'cellItem', key: colIndex },
										cell
									)
								);
							})
...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	function computerMove(board) {
		...
		var rotatedBoard = (0, _utils.rotateBoard)(board);
		var maxPoints = 0,
		    maxCoord,
		    currentPoints;
		var defenseCoord;
		for (var i = 0; i < board.length; i++) {
			for (var j = 0; j < board.length; j++) {
				...
				if (!(0, _utils.isValidMove)(coord, board)) {
					continue;
				}
				var diagArrays = (0, _utils.getDiagnolArrays)(board);
				var diagFull = ((0, _utils.isRowComplete)(diagArrays[0]) || (0, _utils.isRowComplete)(diagArrays[1])) && board[1][1] === 'X';
				...
				var diagNum = (0, _utils.isOnDiagonal)(coord, board); // false if not a diag

				// short curcuit lookup, or grab proper array based on diagNum (0/1)
				var diag = diagNum === false ? null : (0, _utils.getDiagnolArrays)(board)[diagNum];

				// check win
				if ((0, _utils.hasExactlyTwoVals)(row, 'O') || (0, _utils.hasExactlyTwoVals)(col, 'O') || (0, _utils.hasExactlyTwoVals)(diag, 'O')) {
					return coord;
				}

				// check for immediate loss
				if ((0, _utils.hasTwoInRow)(row) || (0, _utils.hasTwoInRow)(col) || (0, _utils.hasTwoInRow)(diag)) {
					defenseCoord = coord;
				}

				if ((0, _utils.isValidMove)({ row: 1, col: 1 }, board)) {
					return { row: 1, col: 1 };
				}
				//console.log('Row:',row, 'Col:',col);
				// otherwise use points to choose location
				var rowPoints = (0, _utils.countItems)(row, 'O') + (0, _utils.countItems)(row, 'X');
				var colPoints = (0, _utils.countItems)(col, 'O') + (0, _utils.countItems)(col, 'X');
				console.log('Column: ', col);
				var diagPoints = 0;

				if (diag !== null) {
					console.log(diag);
					diagPoints = (0, _utils.countItems)(diag, 'O') + (0, _utils.countItems)(diag, 'X');
				}
...
/***/ function(module, exports) {
	...
	exports.getDiagnolArrays = getDiagnolArrays;
	exports.rotateBoard = rotateBoard;
	exports.hasExactlyTwoVals = hasExactlyTwoVals;
	exports.isGameWon = isGameWon;
	exports.isCatsGame = isCatsGame;
	exports.isValidMove = isValidMove;
	exports.isOnDiagonal = isOnDiagonal;
	exports.countItems = countItems;
	exports.hasTwoInRow = hasTwoInRow;
	...
	function getDiagnolArrays(board) {
		var diag1 = board.map(function (row, i) {
			return row[i];
		});
		var diag2 = board.map(function (row, i) {
			return row[2 - i];
		});
		return [diag1, diag2];
	}
	...
	function hasExactlyTwoVals(arr, val) {
	...
	function isGameWon(board, char) {
		var rotatedBoard = rotateBoard(board);
		var checkRow = isRowCompleteWithChar.bind(this, char);

		var _getDiagnolArrays = getDiagnolArrays(board),
		    _getDiagnolArrays2 = _slicedToArray(_getDiagnolArrays, 2),
		    diag1 = _getDiagnolArrays2[0],
		    diag2 = _getDiagnolArrays2[1];
	...
	function isValidMove(coord, board) {
		return board[coord.row][coord.col] === ' ';
	}

	function isOnDiagonal(coord, board) {
		var row = coord.row;
		var col = coord.col;
		if (row === col) {
			return 0;
	...
	function hasTwoInRow(arr) {
		if (!Array.isArray(arr)) return false;
		return arr.filter(function (x) {
			return x === 'X';
		}).length === 2 || arr.filter(function (x) {
			return x === 'O';
		}).length === 2;
	}
